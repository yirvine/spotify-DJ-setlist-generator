...
def generate_tonal_set(df, num_songs, first_song=None):
    ...
    for i in range(1, num_songs):
        ...
        if section_idx in {0}:  # First section
            compatible_songs.loc[:, 'Score'] = (-1.5 * compatible_songs['Hype'] 
        ...
        elif section_idx in {2,4}:  # Middle and last sections
            compatible_songs.loc[:, 'Score'] = (0.4 * compatible_songs['Hype']
            + 1 * compatible_songs.apply(lambda x: utils.calculate_compatibility(dj_set[-1], x), axis=1)
            + 1 * compatible_songs.apply(lambda x: utils.calculate_hype_compatibility(dj_set[-1], x), axis=1) 
            )           
...
def fitness_tonal(dj_set):
    ...
    for idx, (start, end) in enumerate(sections):
        if idx == 0:  # First section
            section_hype = -1.5 * sum(song['Hype'] for song in dj_set[start:end])
            section_compatibility = sum(utils.calculate_compatibility(dj_set[i], dj_set[i + 1]) for i in range(start, end - 1)) + sum(utils.calculate_hype_compatibility(dj_set[i], dj_set[i + 1]) for i in range(start, end - 1))
            section_scores += section_hype + section_compatibility
        elif idx in {2, 4}:  # Middle and last sections
    ...
    fitness = 0.2 * total_hype + (transition_compatibility) + hype_compatibility + section_scores
